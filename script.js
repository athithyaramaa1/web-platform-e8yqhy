...
const fiveLetterStrings = strings.filter(function(item){
  return item.length === 5;
});

console.log(fiveLetterStrings);
...
const word = alphabets.reduce(function(result, item){
  return result + item;
})
...
const evenno = number.find(function(item){
  return item%2 === 0;
});
...
const seven = ranks.findIndex(function(item){
  return item === 7;
});
...
//3:8
const num = [2, 4, 6, 8];
const xyz = num.forEach(function(item, array){
...
const checking = positive.every(function(item){
  return item > 0;
})
...
console.log(cars[2].name, cars[0].color[1])
console.log(cars[1].name, cars[1].color[2], cars[0].capacity)
...
const Ajay = `Mr ${person.name} travels by ${person.vehicles['plane']} called Air123`;
console.log(Ajay);
